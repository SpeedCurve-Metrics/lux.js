...
describe("LUX SPA user timing", () => {
  ...
  test("user timing marks and measures are recorded", async () => {
    ...
    // measure, we wait 50ms between the mark and measure, which should make the measure ~50ms.
    ...
    expect(parseInt(userTiming["test-measure"])).toBeGreaterThanOrEqual(50);
    expect(parseInt(userTiming["test-measure"])).toBeLessThan(60);
  });
  ...
  test("user timing marks are relative to the previous LUX.init call", async () => {
    ...
    expect(parseInt(userTiming["test-mark"])).toBeGreaterThanOrEqual(50);
    expect(parseInt(userTiming["test-mark"])).toBeLessThan(60);
  });
