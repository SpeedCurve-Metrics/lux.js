...
function _getNodeSelector(node: Node, selector = ""): string {
  try {
    if (
      selector &&
      (node.nodeType === 9 || selector[PROPS._length] > MAX_SELECTOR_LENGTH || !node.parentNode)
    ) {
      // Final selector.
      return selector;
    }
    ...
    if (el.id) {
      ...
      while ((name + classes)[PROPS._length] > MAX_SELECTOR_LENGTH) {
      ...
      if (el.parentNode) {
        ...
        if (selectorWithParent[PROPS._length] < MAX_SELECTOR_LENGTH) {
